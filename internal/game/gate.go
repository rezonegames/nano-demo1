...
func (g *GateService) Register(s *session.Session, req *proto.RegisterGameReq) error {
	...
	return s.Response(&proto.LoginToGameResp{Player: ConvPlayerToProtoPlayer(pp)})
}

func (g *GateService) Login(s *session.Session, req *proto.LoginToGameReq) error {
	uid := req.UserId
...
func (g *GateService) rooms(s *session.Session, _ *proto.Empty) error {
	return s.Response(&proto.GetRoomListResp{RoomList: ConvRoomListToProtoRoomList(config.ServerConfig.Rooms)})
}
